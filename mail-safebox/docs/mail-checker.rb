...
Gmail.new(params['mailbox']['user_name'], params['mailbox']['password']) do |gmail|
  return File.write('safebox_remaining_days', params['safebox_max_days']) if gmail.label("[Gmail]/Messages envoy&AOk-s").count(:after => Date.parse(yesterday.strftime('%Y-%m-%d'))) == 0
end

Gmail.new(params['safebox']['user_name'], params['safebox']['password']) do |gmail|
  gmail.deliver do
    to params['user_personnal_mail']
